...
- ~~Deployment target (local only, cloud VM, containers on a PaaS)?~~ **Resolved, superseding Phase 7's original "local Docker" choice: Railway (backend) + Vercel (frontend)** — see [deployment-plan.md](deployment-plan.md) for the full walkthrough. The `Dockerfile` from Phase 7 is reused as-is (Railway builds from it directly); it needed one fix (binding to Railway's `$PORT` instead of a hardcoded 8000) plus a new `frontend/web/config.js` mechanism so the build-step-free static frontend can be told the deployed backend's URL.
